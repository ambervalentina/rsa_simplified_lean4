...
-- helper lemmas
variable (a b n : ℕ)
lemma mod_pow_mod_eq_pow_mod (ha: a % n = b): a ≡ b [MOD n] := by
  rw [← ha]
  apply Nat.ModEq.symm
  exact Nat.mod_modEq a n

lemma pow_mod_eq_mod_pow : ((a % n) ^ b) % n = (a ^ b) % n := by
  change (a % n) ^ b ≡ (a ^ b) [MOD n] -- (a % n) ^ b ≡ a ^ b [MOD n]
  apply Nat.ModEq.pow
  exact Nat.mod_modEq _ _

lemma mul_mod_eq_mod_mul_mod : (a * b) % n = ((a % n) * (b % n)) % n := by
...
lemma exists_multiple_add_one (h : p ≡ 1 [MOD n]) (hn : 0 < n) (hp : 1 ≤ p)
    : ∃ k : ℕ, p = 1 + k * n := by
  -- convert modulo to |
...
theorem pow_totient_mul_eq_one (m k p q : ℕ) -- Chinese Remainder Theorem
    (prime_p : Nat.Prime p)
    (prime_q : Nat.Prime q)
    (diff_pq : p ≠ q)
...
theorem rsa_correctness (prime_p: Nat.Prime p) (prime_q: Nat.Prime q)
  ...
  obtain ⟨k, hk⟩ := exists_multiple_add_one e ((p-1) * (q-1)) pub_e_coprime_totient hphi_n he
  ...
  rw [mul_mod_eq_mod_mul_mod]
  -- m ^ 1 % (p*q) = m % (p*q)
  rw [pow_totient_mul_eq_one m k p q prime_p prime_q diff_pq]
